...
| BETWEEN A AND B | A**以上**B**以下**のデータを取得する | `SELECT * FROM users WHERE age BETWEEN 20 AND 30;` |
...
- 文字列や日付は `'` (シングルクォート) で囲む (`"` (ダブルクォート)でも動くこともある)
    ...
    <li class="border-bottom">
        <ul>
            <li>
                ...
                <b>REDOログファイル</b>に「コミット済み」のレコードを追記して確定させる<br>
                ※注意 : この時点ではディスク上のデータベース本体ファイルはまだ<b>書き換わっていない</b><br><br>
            </li>
            <li>
                問題があれば<b>ROLLBACK</b>する<br>
                <b>UNDOログ</b>を使ってバッファプール上のデータを更新前の状態に戻す<br>
                ※注意 : この時点ではディスク上のデータベース本体ファイルはまだ<b>書き換わっていない</b>ため、不整合は発生しない
            </li>
        </ul>
    </li>
    <li class="border-bottom">
...
<ul>
    <li>
        バックグラウンドのチェックポイント処理により、定期的にバッファプール上の変更済みデータ(ダーティページ)がディスク上の<b>データベース本体ファイル</b>へまとめて書き込まれる
    </li>
    <li>
        障害発生時は、<b>バックアップファイル(フルバックアップ)</b>を土台にして<b>REDOログ(アーカイブログ)</b>を時系列に適用しロールフォワードすることで、障害直前の状態まで復元する
    </li>
</ul>
